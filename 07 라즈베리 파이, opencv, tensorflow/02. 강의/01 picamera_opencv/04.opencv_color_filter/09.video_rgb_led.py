import cv2
import numpy as np
from gpiozero import LED


# LED 설정
red_led = LED(14)
green_led = LED(15)
blue_led = LED(18)
yellow_led = LED(17)

lower = {
    "red": (170, 100, 100),
    "green": (50, 100, 100),
    "blue": (110, 100, 100),
    "yellow": (10, 100, 100)
}

upper = {
    "red": (180, 255, 255),
    "green": (70, 255, 255),
    "blue": (130, 255, 255),
    "yellow": (40, 255, 255)
}

cap = cv2.VideoCapture(0)
cap.set(3, 320)
cap.set(4, 240)

while True:
    ret, img = cap.read()
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)

    # red
    red = cv2.inRange(hsv, lower["red"], upper["red"])
    red = np.array(red)
    red = np.reshape(red, (240, 320,))
    red_count = np.count_nonzero(red)
    print(f"red: {red_count}")
    if red_count > 3000:
        red_led.on()
    else:
        red_led.off()

    # green
    green = cv2.inRange(hsv, lower["green"], upper["green"])
    green = np.array(green)
    green = np.reshape(green, (240, 320,))
    green_count = np.count_nonzero(green)
    print(f"green: {green_count}")
    if green_count > 3000:
        green_led.on()
    else:
        green_led.off()

    # blue
    blue = cv2.inRange(hsv, lower["blue"], upper["blue"])
    blue = np.array(blue)
    blue = np.reshape(blue, (240, 320,))
    blue_count = np.count_nonzero(blue)
    print(f"blue: {blue_count}")
    if blue_count > 3000:
        blue_led.on()
    else:
        blue_led.off()

    # mask all
    total = red + green + blue
    total = np.reshape(total, (240, 320,))
    mask = cv2.bitwise_and(img, img, mask=total)

    cv2.imshow("img", img)
    cv2.imshow("mask", mask)

    # press 'ESC' to quit
    k = cv2.waitKey(30) & 0xff
    if k == 27:
        break

cap.release()
cv2.destroyAllWindows()
